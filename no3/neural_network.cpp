...
class TowLayerNetwork
{
      ...
      public:
        ...
        std::map<std::string, mat::Matrix<VAL_TYPE>> gradient(mat::Matrix<VAL_TYPE> x, mat::Matrix<VAL_TYPE> t)
        {
                ...
                mat::Matrix<VAL_TYPE> dy(dy_double.width, dy_double.height, dy_double.channel);
                ...
                mat::Matrix<VAL_TYPE> W2 = z1.transpose().dot(dy);
                grads.insert(std::make_pair("W2", W2));

                mat::Matrix<VAL_TYPE> b2 = dy.sum(1);
                grads.insert(std::make_pair("b2", b2));

                mat::Matrix<VAL_TYPE> dz1 = dy.dot(params->at("W2").transpose());
                mat::Matrix<VAL_TYPE> da1 = a1.apply(relu_gradient) * dz1;
                mat::Matrix<VAL_TYPE> W1 = x.transpose().dot(da1);
                grads.insert(std::make_pair("W1", W1));

                mat::Matrix<VAL_TYPE> b1 = da1.sum(1);
...
int main(void)
{
        ...
        mat::Matrix<VAL_TYPE> test_batch_data(test_data.width * test_data.height, 1000);
        mat::Matrix<VAL_TYPE> test_batch_label(1, 1000);
